...
def build_trans_prompt(source_lang, trans_lang, data):    
    ...
    prompts = [f"Translate '{string.strip()}' into {trans_lang}. I need only one translation choice. Do not add other string. Do not remove any string. Do not drop original sentence. Keep original format. " for string in data if string.strip()]
...
def main():    
        ...
        if prompt:
            ...
            if "Translate '' from" in prompt:
                ...
                if '<a href=' in prompt:
                    ...
                    link_url = ''
                    link_text = ''
                    if re.search(r'href="([^"]*)"', html_string):
                        link_url = re.search(r'href="([^"]*)"', html_string).group(1)
                    if re.sub(r'<.*?>', '', html_string):
    ...
    add_font_style_result_path = 'test_result_font_styled_70b.docx'
    ...
    add_para_style_result_path = 'test_result_para_styled_70b.docx'
    ...
    hyperlink_result_path = 'test_result_hyperlink_70b.docx'
